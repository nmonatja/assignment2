...
public class CultureboxItem extends DbItem implements InventoryItem {
	...
	public CultureboxItem(String code, String desc, int quant, float cost) {
		product_code = code;
		description = desc;
		quantity = quant;
		price = cost;
	...
	public Boolean update(String product_code, String description, Integer quantity, Float price) {
		...
		sql = "UPDATE Cultureboxes SET "+sql+" WHERE productid = '"+this.product_code+"'";
	...
	public Boolean remove() {
		...
		int retVal = 0;
		String sql = "DELETE from Cultureboxes WHERE productid = '"+this.product_code+"'";
