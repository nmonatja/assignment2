...
public class ReferenceMaterialItem extends DbItem implements InventoryItem {
	...
	public ReferenceMaterialItem(String code, String desc, int quant, float cost) {
		product_code = code;
		description = desc;
		quantity = quant;
		price = cost;
	...
	public Boolean update(String product_code, String description, Integer quantity, Float price) throws UpdateException {
		...
		sql = "UPDATE ReferenceMaterials SET "+sql+" WHERE productid = '"+this.product_code+"'";
	...
	public Boolean remove() throws DeleteException {
		...
		int retVal = 0;
		String sql = "DELETE from ReferenceMaterials WHERE productid = '"+this.product_code+"'";
